...
def pytest_sessionfinish(session, exitstatus):
    ...
    if platform.system() == "Windows":
        allure_cmd = r"C:\Users\USER\scoop\apps\allure\current\bin\allure.bat"
    else:
        # On Linux/Mac, we usually just call 'allure' if it's in your PATH
        allure_cmd = "allure"

    # ✅ Generate report
    try:
        subprocess.run(
            f'{allure_cmd} generate "{results_dir}" -o "{output_dir}" --clean',
            shell=True,
            check=True
        )
        print(f"\n✅ Allure report generated at: {output_dir}\n")
    except subprocess.CalledProcessError:
        print("\n❌ Failed to generate Allure report. Is Allure installed on this system?\n")
